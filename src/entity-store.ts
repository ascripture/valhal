...
import { nestedPathValue, resettable } from "./util";
import { Store, StoreData } from "./store";

export interface EntityStoreData<ENTITY, ID = any> extends StoreData<ID, ENTITY> {
    data: Map<ID, ENTITY>;
}

export class EntityStore<ENTITY, ID = any, META = any> implements Store<ENTITY, ID, META> {
    ...
    constructor(
        ...
        const { observable, subject, reset } = resettable(() => new ReplaySubject<EntityStoreData<ENTITY>>(1));
    ...
    selectEntity(id: ID) {
        return this.asObservable().pipe(map(store => store?.data?.get(id)), distinctUntilChanged());
    }
    ...
    private getId(entity: Partial<ENTITY> | undefined): ID {
        const id = nestedPathValue<ID>(entity, this.config.idPath.values());
